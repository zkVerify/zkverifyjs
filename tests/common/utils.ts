...
export function getProofFilenameComponents(
...
export const loadVerificationKey = (proofOptions: ProofOptions): string => {
  const fileName =
    getProofFilenameComponents(proofOptions).join('_').toLowerCase() + '.json';
  const dataPath = path.join(__dirname, 'data', fileName);
...
export const performVerifyTransaction = async (
  ...
  try {
    ...
    const verifyTransaction = async () => {
      const verify = dispatchBuilder(
        session.verify(accountAddress),
        proofOptions,
      );
    ...
    throw new Error(
      `Failed to execute transaction. See logs for details: ${errorMessage}`,
    );
  }
};
...
export const performVKRegistrationAndVerification = async (
  ...
  const register = dispatchBuilder(
    session.registerVerificationKey(accountAddress),
    proofOptions,
  );

  const {
    events: registerEvents,
    transactionResult: registerTransactionResult,
  } = await register.execute(vk);
  ...
  const vkTransactionInfo: VKRegistrationTransactionInfo =
    await registerTransactionResult;
  validateVKRegistrationTransactionInfo(
    vkTransactionInfo,
    proofOptions.proofType,
  );
  ...
  const { events: verifyEvents, transactionResult: verifyTransactionResult } =
    await verify.withRegisteredVk().execute({
      proofData: {
        proof: proof,
  ...
  const verifyResults = handleCommonEvents(
    verifyEvents,
    proofOptions.proofType,
    TransactionType.Verify,
  );
...
const retryWithDelay = async <T>(
  ...
  while (attempt < retries) {
    try {
      ...
      console.warn(
        `Retrying after error: ${errorMessage}. Attempt ${attempt} of ${retries}`,
      );
...
export const performRegisterDomain = async (
  ...
  const { events, transactionResult } = session.registerDomain(
    aggregationSize,
    queueSize,
    deliveryOptions,
    signerAccount,
...
export const performHoldDomain = async (
  ...
  const { events, transactionResult } = session.holdDomain(
    domainId,
    accountAddress,
  );
  ...
  validateDomainTransactionInfo(
    transactionInfo,
    expectRemovable ? 'Removable' : 'Hold',
  );
...
export const performUnregisterDomain = async (
  ...
  const { events, transactionResult } = session.unregisterDomain(
    domainId,
    accountAddress,
  );
...
export const performAggregate = async (
  ...
  const { events, transactionResult } = session.aggregate(
    domainId,
    aggregationId,
    accountAddress,
  );
...
export function validateAggregateStatementPathResult(
  result: unknown,
): asserts result is AggregateStatementPathResult {
  ...
  if (
    !(
      'root' in result &&
      'proof' in result &&
      'numberOfLeaves' in result &&
      'leafIndex' in result &&
      'leaf' in result
    )
  ) {
    throw new Error(
      'Result object is missing one or more required properties: root, proof, numberOfLeaves, leafIndex, leaf.',
    );
  }

  const { root, proof, numberOfLeaves, leafIndex, leaf } = result as Record<
    string,
    unknown
  >;
  ...
  if (
    typeof numberOfLeaves !== 'number' ||
    !Number.isInteger(numberOfLeaves) ||
    numberOfLeaves < 1
  ) {
    throw new Error(
      `Invalid 'numberOfLeaves': Received ${JSON.stringify(numberOfLeaves)}. Must be a positive integer >= 1.`,
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Invalid 'leafIndex': Received ${JSON.stringify(leafIndex)}. Must be a non-negative integer.`,
    );
  }
...
export const formatProofConfigDetails = (
  proofOptions: ProofOptions,
): string => {
