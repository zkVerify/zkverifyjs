  ...
  switch (transactionType) {
    case TransactionType.Verify:
      ...
      if (verifyInfo.attestationId) {
        ...
        safeEmit(emitter, ZkVerifyEvents.ErrorEvent, { ...verifyInfo, error: 'Finalized but no attestation ID found.' });
      }
      break;
    ...
    case TransactionType.VKRegistration:
      ...
      if (vkInfo.statementHash) {
        ...
        safeEmit(emitter, ZkVerifyEvents.ErrorEvent, { ...vkInfo, error: 'Finalized but no statement hash found.' });
      }
      break;
    ...
    case TransactionType.DomainRegistration:
      ...
      if (domainInfo.domainId !== undefined) {
        safeEmit(emitter, ZkVerifyEvents.NewDomain, { domainId: domainInfo.domainId });
        safeEmit(emitter, ZkVerifyEvents.Finalized, domainInfo);
      } else {
        safeEmit(emitter, ZkVerifyEvents.ErrorEvent, { ...domainInfo, error: 'Finalized but no domain ID found.' });
      }
      break;

    case TransactionType.DomainHold:
    case TransactionType.DomainUnregister:
      const domainStateInfo = transactionInfo as DomainHoldTransactionInfo | DomainUnregisterTransactionInfo;
      if (domainStateInfo.domainState) {
        safeEmit(emitter, ZkVerifyEvents.Finalized, domainStateInfo);
      } else {
        safeEmit(emitter, ZkVerifyEvents.ErrorEvent, { ...domainStateInfo, error: 'Finalized but no domain state returned.' });
      }
      break;
 ...
 */
const initializeTransactionInfo = (transactionType: TransactionType, options: VerifyOptions): TransactionInfo => {
  ...
  switch (transactionType) {
    case TransactionType.Verify:
      return { ...baseInfo, proofType: options.proofOptions?.proofType, attestationId: undefined, leafDigest: null, attestationConfirmed: false } as VerifyTransactionInfo;

    case TransactionType.VKRegistration:
      return { ...baseInfo, proofType: options.proofOptions?.proofType, statementHash: undefined } as VKRegistrationTransactionInfo;
    ...
    case TransactionType.DomainHold:
    case TransactionType.DomainUnregister:
      return { ...baseInfo, domainState: '' } as DomainHoldTransactionInfo | DomainUnregisterTransactionInfo;
  ...
  return new Promise((resolve, reject) => {
    ...
    const finalizeTransaction = async (result: SubmittableResult) => {
      ...
      try {
        ...
        if (transactionType === TransactionType.Verify && options.waitForNewAttestationEvent && (transactionInfo as VerifyTransactionInfo).attestationId) {
          try {
    ...
    performSignAndSend(submitExtrinsic, account, signer ? { signer, nonce: options.nonce ?? -1 } : {}, async (result) => {
      if (transactionInfo.status === TransactionStatus.Error) return;

      try {
        if (result.status.isBroadcast) {
          safeEmit(emitter, ZkVerifyEvents.Broadcast, { txHash: result.txHash.toString() });
        }
